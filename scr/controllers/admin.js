...
module.exports = {
  ...
  rejectTeacher: async (req, res) => {
    ...
    const teacher = await new TeacherService({}).rejectedByAdmin(
      teacherId,
      msg
    );
  ...
  getAllTeacher: async (req, res) => {
    const teachers = await JoinRequists.find(
      {},
      { teacherToPlatform: 1, _id: 0 }
    ).populate({
    ...
    const filteredTeachers = teachers.filter(
      (item) => Object.keys(item).length > 0 && item.teacherToPlatform && item.teacherToPlatform.teacherId
    );
    //console.log(result);

    responseSender(res, filteredTeachers);
  },
