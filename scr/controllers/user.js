...
module.exports = {
  ...
  update: async (req, res) => {
    ...
    if (body.password || body.email) {
      ...
      await new CredentialService({ ...body }).changePassword(
        user.credentialId
      );
    }
  ...
  enroll: async (req, res) => {
    ...
    if (user.wallet > course.cost) {
      user.wallet -= course.cost;
      await user.save();
      const Updateuser = await new UserService({}).enroll(courseId, userId);
      responseSender(res, Updateuser);
    } else {
      responseSender(res, "can't buy the course", "fail", 402);
    }
  },
  ...
  addToProgress: async (req, res) => {
    ...
    if (progress && progress.percent === 100) {
        console.log('we acheive to 100% and the course must done')
      const user = await new UserService({}).finishCourse(courseId, userId);
      responseSender(res, user);
    } else {
